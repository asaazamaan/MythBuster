...
function App() {
  ...
  const getSourceIcon = (source) => {
    if (!source) return "📄";
    if (source.source_type === "rag") return "📚"; // RAG/Research source
    if (source.source_type === "web") {
      return source.trusted ? "🔍" : "🌐"; // Trusted vs general web
    }
    return "📄"; // Default
  };
                      ...
                      ? // Show claims with verdicts
                        result.verdicts.map((verdict, index) => (
                          ...
                          >
                            ...
                            {/* Enhanced sources section with type indicators */}
                            {verdict.sources && verdict.sources.length > 0 && (
                              <div className="sources-section">
                                ...
                                <div className="sources-list">
                                  {verdict.sources.map((source, sourceIndex) => (
                                    <div 
                                      key={sourceIndex} 
                                      className={`source-item ${source.trusted ? 'trusted-source' : 'untrusted-source'}`}
                                    >
                                      <div className="source-header">
                                        ...
                                        <span
                                          className={`relevance-badge ${
                                            source.relevance_badge || (source.trusted ? 
                                              (sourceIndex === 0 ? 'primary' : sourceIndex === 1 ? 'secondary' : 'tertiary')
                                              : 'neutral'
                                            )}`}
                                        >
                                          {source.relevance_display || (sourceIndex === 0 ? 'Most Relevant' : sourceIndex === 1 ? 'Moderately Relevant' : 'Supporting Evidence')}
